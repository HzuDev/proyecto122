...
export default () => {
    const { cart, showModal, setShowModal, removeFromCart, updateQuantity, calculateTotal } = useContext(CartContext);
  return (
    <>
      <nav className="font-sans min-h-[60px]  border-b bg-opacity-60 bg-clip-padding p-2 bg-indigo-950">
        <div className="flex flex-wrap items-center justify-between px-10 relative">
          ...
          <ul className="flex max-lg:block max-lg:w-full lg:space-x-6 max-lg:space-y-4 z-10 items-center">
            ...
            <li className="max-lg:border-b max-lg:py-2 hover:rounded hover:bg-indigo-800/80 py-2 px-5 hover:text-gray-600 transition duration-150 ease-out hover:ease-in relative">
            <button onClick={() => setShowModal(!showModal)}
                className="hover:text-indigo-600 text-[15px] font-bold text-white block flex items-center"
            >
        ...
        <div className="absolute text-xs rounded-full -mt-1 -mr-2 px-1 font-bold top-0 right-0 bg-red-700 text-white">{cart.length}</div>

                Carrito
            </button>
            <Modal showModal={showModal} setShowModal={setShowModal} cart={cart} removeFromCart={removeFromCart} updateQuantity={updateQuantity} calculateTotal={calculateTotal} />
            </li>
